...
/*
  TODO maine:
  run through each lesson again
  make loading bar bigger/more noticeable
  prepare examples for each part

  say that I wouldn't be using typescript
  I'll be using React as a backdrop
*/
...
class Service {
  ...
  constructor() {
    const hideLoaderCombo = keyboardCombo(["a", "s", "d"]);
    ...
    showWithDelay
      .pipe(
        switchMap(() =>
          displayLoader.pipe(takeUntil(merge(hideDelayed, hideLoaderCombo)))
        )
      )
      .subscribe();
  }
